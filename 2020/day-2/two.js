...
const validPassword = passwords.filter(({ policy, password }) => {
  const posA = password[policy.min - 1];
  const posB = password[policy.max - 1];

  if (posA === policy.letter && posB === policy.letter) {
    return false;
  }

  if (posA === policy.letter) {
    return true;
  }

  if (posB === policy.letter) {
    return true;
  }
