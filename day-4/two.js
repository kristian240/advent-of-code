...
const fields = {
  mandatory: [
    { name: 'byr', rule: /^(19[2-9][0-9]|200[0-2])$/ },
    { name: 'iyr', rule: /^20(1[0-9]|20)$/ },
    { name: 'eyr', rule: /^20(2[0-9]|30)$/ },
    { name: 'hgt', rule: /^((59|6[0-9]|7[0-6])in|(1([5-8][0-9]|9[0-3]))cm)$/ },
    { name: 'hcl', rule: /^#[0-9a-f]{6}$/ },
    { name: 'ecl', rule: /^(amb|blu|brn|gry|grn|hzl|oth)$/ },
    { name: 'pid', rule: /^\d{9}$/ },
  ],
  optional: ['cid'],
};
...
const passports = input.split('\n\n').map((passportString) => {
  return passportString.split(/\s+/).reduce((passport, passportField) => {
    const [key, value] = passportField.split(':');

    return Object.assign({}, passport, { [key]: value });
  }, {});
});

const isPassportValid = (passport) => {
  const keys = Object.keys(passport).filter((key) => !fields.optional.includes(key));
